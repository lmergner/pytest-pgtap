...
def match_file_name(
    ...
    logger.debug(
        f'Comparing {fname} to {pattern} excluding {default_exclude}')
    return fnmatch.fnmatch(fname, pattern) and \
        not any([fnmatch.fnmatch(fname, ex) for ex in default_exclude])
